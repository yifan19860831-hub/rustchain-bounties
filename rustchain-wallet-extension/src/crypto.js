/**
 * RustChain Wallet - Crypto Utilities
 * Implements Ed25519, BIP39, PBKDF2, and AES-256-GCM using Web Crypto API
 */

// BIP39 Word List (simplified - first 256 words for demo, full list has 2048)
const WORDLIST = [
  ...
  "gun","gym","habit","hair","half","hammer","hamster","hand","handle","hang",
  "happen","happy","harbor","hard","harsh","harvest","hat","have","hawk","hazard",
  ...
  "pilot","pin","pine","pink","pipe","pistol","pitch","pizza","place","planet",
  ...
  "simple","since","sing","siren","sister","sit","situation","six","size","skate",
  ...
  "valley","valve","van","vanish","vapor","various","veast","vector","vehicle","vein",
...
class RustChainCrypto {
  /**
   * Generate a random BIP39 24-word mnemonic phrase
   */
  ...
  static async deriveKeyPair(mnemonic) {
    ...
    // Use seed for Ed25519 key generation
    // In production, use proper BIP32 derivation
    const keyData = new Uint8Array(seed.slice(0, 32));
    
    // Generate Ed25519 key pair from seed
    const jwk = {
      kty: 'OKP',
      crv: 'Ed25519',
      d: this.bytesToBase64Url(keyData),
      x: '' // Will be derived
    };

    // For demo purposes, generate a simple key pair
    // In production, use proper Ed25519 derivation from seed
  ...
  /**
   * Encrypt keystore with password using AES-256-GCM
   */
  static async encryptKeystore(data, password) {
    const encoder = new TextEncoder();
   ...
   */
  static async decryptKeystore(encryptedData, password) {
    const encoder = new TextEncoder();
